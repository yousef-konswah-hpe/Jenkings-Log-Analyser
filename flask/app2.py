...
AI_REQUEST_SEMAPHORE = Semaphore(2)  # Allow max 2 concurrent AI requests
AI_REQUEST_DELAY = 5  # 5 seconds between requests
...
def summarize_chunk(chunk):
    """Summarize a single chunk using LLaMA API with improved error handling"""
    ...
    for attempt in range(max_retries):
        try:
            with AI_REQUEST_SEMAPHORE:
                ...
                # Add random delay to avoid overwhelming the AI model
                if attempt > 0:
                    delay = AI_REQUEST_DELAY * (2 ** attempt) + random.uniform(1, 3)
...
class JenkinsLogProcessor:
    ...
    def process_jenkins_log(self, job_id, jenkins_url=None, username=None, password=None):
        """Process Jenkins log and return analysis directly with better error handling"""
        try:
            ...
            # Add timeout for Jenkins connection
            server = jenkins.Jenkins(jenkins_url, username=username, password=password, timeout=60)
            ...
            print(f"[ERROR] Jenkins error for job {job_id}: {e}")
            return None, f"Jenkins error: {str(e)}"
        except Exception as e:
            print(f"[ERROR] Error in process_jenkins_log for job {job_id}: {e}")
            return None, str(e)
...
def schedule_or_send_email(job_id):
    try:
        ...
        def generate_and_send_summary():
            """Generate latest Jenkins log analysis and send via email with improved error handling"""
            try:
                # Add random delay to spread out concurrent jobs
                initial_delay = random.uniform(0, 30)  # 0-30 seconds
                print(f"[EMAIL] Starting analysis for job {job_name} (delayed by {initial_delay:.1f}s)")
                time.sleep(initial_delay)
                
                print(f"[EMAIL] Generating summary for job: {job_name}")
                
                # Process Jenkins log with better error handling
                result, error = processor.process_jenkins_log(job_id, jenkins_url, username, password)
                if error:
                    print(f"[EMAIL] ❌ Error processing Jenkins log for {job_name}: {error}")
                    ...
                    processor.send_email_report(error_analysis, email, frequency or 'immediate', job_id)
                    return

                # Send email with analysis
                success = processor.send_email_report(result, email, frequency or 'immediate', job_id)
                if success:
                    print(f"[EMAIL] ✅ Summary sent successfully to {email} for job {job_name}")
                else:
                    print(f"[EMAIL] ❌ Failed to send summary to {email} for job {job_name}")
                
            except Exception as e:
                print(f"[EMAIL] ❌ Critical error in generate_and_send_summary for job {job_name}: {e}")
                ...
                try:
                    ...
                    print(f"[EMAIL] ❌ Failed to send error notification email for job {job_name}")
        ...
        if frequency:
            ...
            try:
                ...
                print(f"[EMAIL] Scheduled {frequency} email for job {job_name} to {email} (cron: {cron_expr})")
                return jsonify({
                    'success': True,
